...
import { toNakedPktIso } from "@/utils/datetime";
...
export default function CreateRide() {
  ...
  const vehiclesQuery = useQuery<VehicleDto[]>({
    queryKey: ["vehicles"],
    queryFn: listMyVehicles,
    staleTime: 0,
  });
  const vehicles = vehiclesQuery.data ?? [];
  const loadingVehicles = vehiclesQuery.isLoading || vehiclesQuery.isFetching;
  const vehiclesError = vehiclesQuery.isError
    ? (vehiclesQuery.error as any)?.response?.data?.message ??
      (vehiclesQuery.error as any)?.message ??
  ...
  const handleConfirm = async () => {
    ...
    try {
      await createRide({
        ...
        toAddress: toCity,
        // Build a naked ISO from local-clock components so the user's chosen
        // wall-time travels as PKT regardless of the device timezone.
        // The backend treats naked ISO as PKT.
        departureTime: toNakedPktIso(dateTime),
  ...
  return (
    ...
    >
      ...
      <PickerSheet
        ...
        emptyText={
          vehiclesError
            ? "Could not load vehicles. Tap refresh or add a new one."
...
function PickerSheet({
  ...
  return (
    ...
    >
      ...
      >
        {/* Tap-to-close backdrop above the sheet */}
