...
// Pulls a human-readable message out of the various error shapes our backend
// returns: plain-text BadRequest("..."), { message: "..." }, and ASP.NET
  ...
  if (data && typeof data === "object") {
    ...
    if (data.errors && typeof data.errors === "object") {
      ...
      for (const [field, msgs] of Object.entries(data.errors as Record<string, unknown>)) {
        ...
        for (const m of list) {
          if (typeof m === "string") lines.push(`${field}: ${m}`);
        }
      }
...
// Strict YYYY-MM-DD parser — rejects month > 12, day > daysInMonth, etc.
// Hermes/V8 will silently roll over invalid components in `new Date(str)` on
// some inputs, so we validate components ourselves instead of trusting Date.
function parseDob(dob: string): Date | null {
  ...
  // Round-trip check catches things like Feb 30, Apr 31.
  ...
  async function onSubmit() {
    ...
    try {
      ...
      navigation.replace("OtpVerification", {
        email: res.email ?? fields.email.trim(),
        purpose: "SignupEmail",
        devOtp: res.devOtp,
      });
    } catch (err: any) {
