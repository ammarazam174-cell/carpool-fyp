...
const STATUS_CFG = {
  Approved: { label: "Verified",      icon: "✓",  cls: "bg-green-500/15 text-green-400 border-green-500/30" },
  Pending:  { label: "Pending Approval", icon: "🕐", cls: "bg-amber-500/15 text-amber-400 border-amber-500/30" },
...
export default function DriverProfile() {
  ...
  const save = async () => {
    ...
    if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { setSaveError("Enter a valid email"); return; }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-slate-900 px-4 py-10">
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <div className="bg-gray-800/60 backdrop-blur border border-gray-700/60 rounded-2xl p-6 text-center shadow-xl">
          ...
          <p className="text-gray-500 text-sm mt-0.5 mb-2">Driver</p>

          {/* Rating */}
          <div className="flex items-center justify-center gap-1.5 mb-3">
            <span className="text-yellow-400">⭐</span>
            <span className="text-white font-semibold">{(profile.rating ?? 0).toFixed(1)}</span>
          ...
          <div className={`flex items-start gap-3 px-4 py-3 rounded-xl border text-sm ${
            ...
            <div>
              <p className="font-semibold">
                {status === "Rejected" ? "Profile Rejected" : "Pending Admin Approval"}
              </p>
              <p className="text-xs mt-0.5 opacity-80">
                {status === "Rejected"
                  ? "Your documents were rejected. Please contact support."
                  : "Your profile is under review. Create Ride is disabled until approved."}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="bg-gray-800/60 backdrop-blur border border-gray-700/60 rounded-2xl overflow-hidden shadow-xl">
          ...
          <div className="divide-y divide-gray-700/40">
            ...
            {/* Date of Birth — editable; shows age inline in view mode */}
            ...
            <div className="flex items-center gap-3 px-5 py-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {editMode ? (
                  ...
                  <p className="text-white font-medium">{profile.gender ?? <span className="text-gray-500">Not set</span>}</p>
                )}
              </div>
        ...
        {profile.isProfileComplete ? (
          <div className="bg-gray-800/60 backdrop-blur border border-gray-700/60 rounded-2xl overflow-hidden shadow-xl">
            ...
            <div className="divide-y divide-gray-700/40">
              ...
              <DocRow icon="📋" label="Driving License" url={profile.licenseImageUrl} />
            </div>
          </div>
        ) : (
