...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    setError("");
    setLoading(true);
    // Drop any stale token before the login attempt so a leftover
    // localStorage entry can't poison the request.
    ...
    try {
      ...
      // 403 with needsEmailVerification = account exists but email unverified.
      // No OTP page on the web client yet, so surface the actual message
      // instead of pretending the password is wrong.
      if (status === 403 && payload?.needsEmailVerification) {
        setError(
          payload.message ??
      ...
      if (status === 401) {
        ...
        setError("Request timed out. Check that the backend is running.");
      } else if (err?.message) {
